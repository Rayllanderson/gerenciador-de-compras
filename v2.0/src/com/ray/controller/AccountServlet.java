                ...
                if (action.equals("editar")) {
                    if (UserValidation.idIsValid(request, id)) { // verificando se o id � de fato o id do user logado
                        if (UserValidation.fieldsAreNotEmpty(name, username)) { // validando os campos
                            redirect(request, response, 400, "Um ou mais campos est�os vazios");
                        } else {
                            // verificando se modificou pra evitar requisi��o desnecess�ria
                            if (UserValidation.userIsModified(name, username, foto64, request)) {
                                ...
                                redirect(request, response, 200, "Nenhuma altera��o foi detectada.");
                            }
                        }
                    } else { // id n�o � o mesmo
                        redirect(request, response, 400, "Ocorreu um erro. Por favor, atualize a p�gina e se o problema persistir, " +
                                "fa�a login novamente.");
                    }
                } else if (action.equals("change-password")) {
                    changePassword(request, response);
                } else {// se mudar o parametro de action
                    redirect(request, response, 400, "Ocorreu um erro. Por favor, atualize a p�gina.");
                }
            } else { // se nao for editar, simplemtente volta tudo
                request.getRequestDispatcher("account.jsp").forward(request, response);
            }
        } catch (EntradaInvalidaException e) { // upou arquivo inv�lido
            redirect(request, response, 400, e.getMessage());
        } catch (MyLoginException e) { // username ja existente
            redirect(request, response, 409, "O username escolhido j� est� em uso. Tente usar outro");
    ...
    private void changePassword(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (UserValidation.passwordIsValid(new1, new2)) { // senhas se batem
            ...
            try {
                ...
                response.getWriter().write("A senha atual digitada n�o corresponde");
            } catch (RuntimeException e) {
                e.printStackTrace();
            ...
            response.setStatus(400);
            response.getWriter().write("As novas senham n�o correspodem");
        }
    }
}
