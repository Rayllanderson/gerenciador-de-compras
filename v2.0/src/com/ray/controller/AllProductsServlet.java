...
public class AllProductsServlet extends HttpServlet {
    ...
    private void salvarProduto(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        try {
            ...
            if (cat == null) { // verificando se o user selecionou uma categoria
                throw new CategoriaInexistenteException("Selecione uma categoria v�lida");
            } else {
                startServiceAndRepository(request, response);
                // setando os valores
                ...
                // verifica��o pra ver se d� update ou se insere
                if (p.getId() == null) {
            ...
            setResponseBody(request, response, "H� um ou mais campos com caracteres inv�lidos.", HttpServletResponse.SC_BAD_REQUEST);
            // existe a chance da pessoa editar o html e mudar o id, ent�o n�o vamos
            // permitir caso a lista nao pertencer ao usuario
        } catch (CategoriaInexistenteException e) {
            setResponseBody(request, response, e.getMessage(), 422);// dados foram compreendidos, mas n�o s�o v�lidos.
        } catch (EntradaInvalidaException e) {
            setResponseBody(request, response, e.getMessage(), 400);
        } catch (NullPointerException e) { // cair� aqui caso o user tentar editar algo no html
    ...
    private void todosProdutosDoUsuario(HttpServletRequest request, HttpServletResponse response) throws ServletException,
            IOException {
    ...
    private void listarComprados(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
    ...
    private void listarNaoComprados(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
     ...
     * Ajax
     *
     * @param mensagem - mensagem que ser� enviada como resposta
     * @param codigo - codigo que ser� enviado
     */
    ...
    private String parseNumber(String value) {
        ...
        return valorParse.replaceAll("\\,", "."); // agora s� sobra a virgula, dai s� mudar pra .
    }

    private void startServiceAndRepository(HttpServletRequest request, HttpServletResponse response) throws IOException {
