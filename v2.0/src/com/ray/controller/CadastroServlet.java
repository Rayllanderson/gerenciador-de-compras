            ...
            if (fieldsAreNotEmpty) {
                if (passwordIsValid) {
                    try {
                        ...
                        setResponse(response, HttpServletResponse.SC_OK, "Cadastro realizado com sucesso! Fa�a login para continuar");
                    } catch (MyLoginException e) { // username ja em uso
                        setResponse(response, HttpServletResponse.SC_CONFLICT, e.getMessage());
                    }
                } else {
                    setResponse(response, HttpServletResponse.SC_BAD_REQUEST, "As senhas n�o correspodem");
                }
            } else {
                setResponse(response, HttpServletResponse.SC_BAD_REQUEST, "Um ou mais campos est�o vazios");
            }
        } catch (Exception e) {
