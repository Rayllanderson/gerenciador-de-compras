...
public class FormatarTabela {
    ...
    public static void printInvoiceHeader(int maxLenName, int maxLenPrice) {
        System.err.println(String.format("%3s %3s %-" + maxLenName + "s " + "%5s %4s %5s %5s %4s %4s", "ID", "|", "Nome", "|", "  " +
                "Pre�o Estipulado(R$)", "|", "  Pre�o Real(R$)", "|", "  Comprado"));
        System.out.println(String.format("%s",
                "----------------------------------------------------------------------------------------------------------------"));
    }

    public static void printInvoice(Product p, int maxLenName, int maxLenPrice, int id) {
        System.out.println(String.format("%3s %3s %-" + maxLenName + "s " + "%" + maxLenPrice + "s %22s " + "%" + maxLenPrice + "s " +
                "%12s %8s %8s", id, "|", p.getNome(), "|", formatar.format(p.getPrecoEstipulado()), "|",
                formatar.format(p.getPrecoReal()), "|", p.comprado()));
    }
